...
router.get('/info', isAuthenticated, async (req, res) => {
    try {
        ...
        const user = await models.User.findByPk(req.user.id, { attributes: ['id', 'username', 'email', 'credits', 'createdAt'] });
        if (!user) return res.status(404).json({ error: 'User not found' });
        res.json({ user });
    } catch (error) {
...
router.get('/transactions', isAuthenticated, async (req, res) => {
    try {
        ...
        const models = require('../models');
        const transactions = await models.Transaction.findAll({ where: { userId: req.user.id }, order: [['createdAt', 'DESC']], limit: 50 });
